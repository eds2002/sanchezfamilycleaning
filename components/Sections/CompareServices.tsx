...
const tiers = [
  ...
  },
  {
    ...
    priceMonthly: 29,
    description:
      "Business use this if they need a deep cleaning of their building",
          ...
          {tiers.map((tier, tierIdx) => (
            <section key={tier.name}>
              <div className="mb-8 lg:px-4">
                ...
                <p className="max-w-xs mt-4 text-sm sm:text-base lg:text-lg ">
                  {tier.description}
                </p>
                <div className="mt-4">
                  <Link href={tier.handle}>
                    <Button theme="primary" fullWidth={true} className="py-3">
                      <p className="">Get a quote for {tier.name}</p>
                    </Button>
                  </Link>
                </div>
              </div>
                ...
                <table key={section.name} className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-400/50">
                    {section.features.map((feature: any) => (
                      <tr
                        key={feature.name}
                        className="border-t border-slate-400/50"
                      >
                        <th
                          className="px-4 py-5 text-sm font-normal text-left "
                          scope="row"
                        >
                          {feature.name}
                        </th>
                        <td className="py-5 pr-4">
                          {typeof feature.tiers[tier.name] === "string" ? (
                            <span className="block text-sm text-right text-white/70">
        ...
        <div className="hidden lg:block">
          <table className="w-full h-px table-fixed">
            ...
            <tbody className="border-gray-200 divide-y divide-slate-500/50 ">
              <tr className="">
                <th
                  className="py-6 text-sm font-medium text-left align-top "
                  scope="row"
                >
                  Service
                </th>
                {tiers.map((tier) => (
                  <td key={tier.name} className="h-full pb-6 align-top">
                    <div className="relative table h-full mx-4 ">
                      <p className="mt-4 mb-16 text-sm opacity-70">
                        {tier.description}
                      </p>
                      <div className="absolute bottom-0 left-0 right-0 mt-4">
                        <Button theme="primary" fullWidth={true}>
              ...
              {sections.map((section) => (
                <Fragment key={section.name}>
                  ...
                  {section.features.map((feature: any) => (
                    <tr key={feature.name}>
                      <th
                        className="px-6 py-5 text-sm font-normal text-left opacity-70"
                        scope="row"
                      >
                        {feature.name}
                      </th>
